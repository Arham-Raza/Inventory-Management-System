...
import {
  getInventoryTemplateBase64,
  parseInventoryExcel,
  bulkAddInventoryItems,
} from "@/actions/inventory"
import type { InventoryImportRow, InvalidImportRow } from "@/lib/inventory-import"
...
type ParseResult = {
  valid: InventoryImportRow[]
  invalid: InvalidImportRow[]
  totalRows: number
}

type ImportResult = { created: number; skipped: string[] }
...
export function BulkUploadPanel() {
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      const { base64, filename } = await getInventoryTemplateBase64()
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await parseInventoryExcel(formData)
  ...
  const handleImport = async () => {
    ...
    setImporting(true)
    try {
      const result = await bulkAddInventoryItems(parseResult.valid)
      setImportResult(result)
      toast.success(`${result.created} item(s) imported — pending admin approval`, {
        description:
          result.skipped.length > 0
            ? `${result.skipped.length} skipped (serial already exists)`
            : undefined,
      })
    } catch (err) {
  ...
  return (
    <Card className="max-w-3xl mx-auto shadow-sm border-gray-200">
      <CardHeader>
        ...
        <CardDescription>
          Import many units at once from a spreadsheet. Every imported item still goes through the same admin approval before it&apos;s sellable at the POS.
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex items-center justify-between gap-4 p-4 rounded-xl bg-gray-50 border border-gray-200">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Download the .xlsx template so your column headers match exactly.
            </p>
          </div>
          <Button
            variant="outline"
              ...
              <div className="rounded-xl border border-gray-200 overflow-hidden">
                ...
                <div className="max-h-80 overflow-y-auto">
                  <Table>
                    ...
                    <TableBody>
                      {parseResult.valid.slice(0, PREVIEW_LIMIT).map((row) => (
            ...
            {!importResult ? (
              ...
              <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 space-y-2">
                <p className="text-sm font-bold text-emerald-800 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4" />
                  {importResult.created} item(s) imported — pending admin approval
                </p>
                {importResult.skipped.length > 0 && (
                  <p className="text-xs text-emerald-700">
                    Skipped (serial already in inventory):{" "}
